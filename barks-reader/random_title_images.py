...
def get_random_image(title_list: List[FantaComicBookInfo]) -> Tuple[str, Titles]:
    title_index = randrange(0, len(title_list))

    comic_book_info = title_list[title_index].comic_book_info
    title = comic_book_info.title

    title_image_file = __get_random_title_image_file(comic_book_info.get_title_str())
    if title_image_file:
...
def get_random_image_file(title_list: List[FantaComicBookInfo]) -> str:
    return get_random_image(title_list)[0]


def __get_random_title_image_file(title: str) -> str:
    num_categories = 6
...
def get_random_title_image(title: str) -> str:
    title_image_file = __get_random_title_image_file(title)
    if title_image_file:
